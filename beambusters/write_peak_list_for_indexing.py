...
if sys.argv[1] == "-":
    ...
    stream = open(
        "/gpfs/cfel/group/cxi/scratch/2021/ESRF-2024-Meents-Mar-ID09/processed/rodria/streams/"
        + sys.argv[1]
        + ".stream",
        "r",
    )
...
def write_agg_file(file_label: str, file_index: int, n_hits: int):
    f = h5py.File(
        f"/gpfs/cfel/group/cxi/scratch/2021/ESRF-2024-Meents-Mar-ID09/processed/rodria/hits/{file_label}-{file_index}-agg_hit.h5",
        "w",
    )
    f.create_dataset("/entry/data/powder", data=powder[: max_ss + 1, : max_fs + 1])
    f.create_dataset(
        "/entry/data/data", data=agg_hits[:n_hits, :, :])
    ...
    f.create_dataset("/entry/data/raw_file_id", data=raw_file_id_list)
    f.create_dataset("/entry/data/detector_shift_y_in_mm", data=shift_y_in_mm_list)
    f.create_dataset("/entry/data/detector_shift_x_in_mm", data=shift_x_in_mm_list)
    f.close()
    ...
    if reading_chunks and n_hits < max_num_hits:
        if line.startswith("End of peak list"):
            ...
            if is_a_hit:
                ...
                shift_x_in_mm_list.append(shift_x_in_mm)
                shift_y_in_mm_list.append(shift_y_in_mm)
                ...
                if n_hits == max_num_hits:
                    ...
                    peakXPosOriginal = np.zeros(
                        (n_hits, max_num_peaks), dtype=np.float32
                    )
                    peakYPosOriginal = np.zeros(
                        (n_hits, max_num_peaks), dtype=np.float32
                    )
                    peakYPosRaw = np.zeros((n_hits, max_num_peaks), dtype=np.float32)
                    peakTotalIntensity = np.zeros(
                        (n_hits, max_num_peaks), dtype=np.float32
                    )
                    ...
                    for i in range(n_hits):
                        ...
                        peakXPosOriginal[i, : peaknPeak_agg[i]] = peakXPosOriginal_agg[
                            i
                        ]
                        peakYPosOriginal[i, : peaknPeak_agg[i]] = peakYPosOriginal_agg[
                            i
                        ]
                        peakYPosRaw[i, : peaknPeak_agg[i]] = peakYPosRaw_agg[i]
                        peakTotalIntensity[i, : peaknPeak_agg[i]] = (
                            peakTotalIntensity_agg[i]
                        )
            ...
            peak_y_pos = fs - shift_y_in_px - 0.5
            peak_y_pos_original = fs - 0.5
            peak_x_pos = ss + shift_x_in_px - 0.5
            peak_x_pos_original = ss - 0.5
            if peak_x_pos <= max_ss and peak_y_pos <= max_fs:
                ...
                peak_list["nPeak"] += 1
                powder[
                    int(round(peak_x_pos, 0)), int(round(peak_y_pos, 0))
                ] += intensity
            ...
            shift_x_in_mm = float(line.split(" = ")[-1])
            shift_x_in_px = shift_x_in_mm * PixelResolution
        elif line.split(" = ")[0] == "header/float//entry/shots/detector_shift_y_in_mm":
            shift_y_in_mm = float(line.split(" = ")[-1])
            shift_y_in_px = shift_y_in_mm * PixelResolution
        elif line.split(" = ")[0] == "header/str//entry/data/raw_file_id":
            raw_file_id_list.append(line.split(" = ")[-1])
